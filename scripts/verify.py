#!/usr/bin/env python3
"""Verify free API models are responding and measure latency.

Usage:
  # Set API keys in .env or as environment variables:
  #   AIONLABS_API_KEY=sk-...
  #   GROQ_API_KEY=gsk_...
  
  python3 scripts/verify.py

Set VERIFY_ALL=1 to run every provider even if the env var is missing.
"""

import json
import os
import sys
import time
import urllib.request
import urllib.error

# Load .env file if present — check script dir first, then cwd
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
for dotenv_path in (os.path.join(_SCRIPT_DIR, ".env"), os.path.join(os.getcwd(), ".env")):
    try:
        with open(dotenv_path) as f:
            for line in f:
                line = line.strip()
                if line and not line.startswith("#") and "=" in line:
                    key, val = line.split("=", 1)
                    os.environ.setdefault(key.strip(), val.strip().strip("\"'"))
    except FileNotFoundError:
        pass
    except Exception:
        pass

PROVIDERS = [
    {
        "name": "AION Labs",
        "base_url": "https://api.aionlabs.ai/v1",
        "model": "aion-labs/aion-3.0",
        "env_key": "AIONLABS_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Agnes AI",
        "base_url": "https://apihub.agnes-ai.com/v1",
        "model": "agnes-1.5-flash",
        "env_key": "AGNESAI_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "AnyAPI AI",
        "base_url": "https://api.anyapi.ai/v1",
        "model": "liquid/lfm-2.5-1.2b-instruct:free",
        "env_key": "ANYAPI_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Auriko",
        "base_url": "https://api.auriko.ai/v1",
        "model": "glm-4.5-flash",
        "env_key": "AURIKO_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Cerebras AI",
        "base_url": "https://api.cerebras.ai/v1",
        "model": "gemma-4-31b",
        "env_key": "CEREBRAS_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Cloudflare Workers AI",
        "base_url": "https://api.cloudflare.com/client/v4/accounts/{account_id}/ai/v1/chat/completions",
        "model": "@cf/google/gemma-2b-it-lora",
        "env_key": "WORKERS_API_KEY",
        "account_id_key": "ACCOUNT_ID",
        "custom_auth": "Bearer",
    },
    {
        "name": "Cohere AI",
        "base_url": "https://api.cohere.com/v2",
        "model": "command-r-08-2024",
        "env_key": "COHERE_API_KEY",
        "api_type": "cohere",
    },
    {
        "name": "ElectronHub",
        "base_url": "https://api.electronhub.ai/v1",
        "model": "gpt-oss-120b",
        "env_key": "ELECTRON_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "FastRouter",
        "base_url": "https://api.fastrouter.ai/api/v1",
        "model": "sarvam/sarvam-105b:free",
        "env_key": "FASTR_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "FreeInference",
        "base_url": "https://freeinference.org/v1",
        "model": "deepseek-v4-flash",
        "env_key": "FREEINFERENCE_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "GitHub Models",
        "base_url": "https://models.inference.ai.azure.com",
        "model": "gpt-4o",
        "env_key": "GITHUB_API_KEY",
        "endpoint": "/chat/completions",
        "custom_auth": "Bearer",
    },
    {
        "name": "Google Gemini",
        "base_url": "https://generativelanguage.googleapis.com/v1beta",
        "model": "models/gemini-flash-lite-latest",
        "env_key": "GOOGLE_API_KEY",
        "api_type": "gemini",
    },
    {
        "name": "Groq API",
        "base_url": "https://api.groq.com/openai/v1",
        "model": "llama-3.3-70b-versatile",
        "env_key": "GROQ_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "HelixMind",
        "base_url": "https://helixmind.online/v1",
        "model": "gpt-oss-20b",
        "env_key": "HELIX_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "HelyxAI",
        "base_url": "https://helyxai.space/v1",
        "model": "DeepSeek-V4-Flash",
        "env_key": "HELYX_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Hugging Face Inference API",
        "base_url": "https://router.huggingface.co/v1",
        "model": "google/gemma-4-26B-A4B-it",
        "env_key": "HF_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Intern AI",
        "base_url": "https://chat.intern-ai.org.cn/api/v1",
        "model": "intern-latest",
        "env_key": "INTERN_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Kilo Code",
        "base_url": "https://api.kilo.ai/api/gateway",
        "model": "kilo-auto/small",
        "env_key": "KILO_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "LLM.Kiwi",
        "base_url": "https://api.llm.kiwi/v1",
        "model": "auto",
        "env_key": "KIWI_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "LLM7.IO",
        "base_url": "https://api.llm7.io/v1",
        "model": "codestral-latest",
        "env_key": "LLM_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "LLMGateway",
        "base_url": "https://api.llmgateway.io/v1",
        "model": "claude-haiku-4-5-free",
        "env_key": "LLMGATE_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "LiteRouter",
        "base_url": "https://api.literouter.com/v1",
        "model": "deepseek-v4-flash:free",
        "env_key": "LITEROUTER_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "MNN AI",
        "base_url": "https://api2.mnnai.ru/v1",
        "model": "deepseek-v3.1",
        "env_key": "MNN_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "MegaNova AI",
        "base_url": "https://api.meganova.ai/v1",
        "model": "BruhzWater/Sapphira-L3.3-70b-0.1",
        "env_key": "MEGANOVA_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Mistral AI",
        "base_url": "https://api.mistral.ai/v1",
        "model": "mistral-code-agent-latest",
        "env_key": "MISTRAL_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Mixlayer",
        "base_url": "https://models.mixlayer.ai/v1",
        "model": "qwen/qwen3.5-4b-free",
        "env_key": "MIXLAYER_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Naga AI",
        "base_url": "https://api.naga.ac/v1",
        "model": "llama-4-scout-17b-16e-instruct:free",
        "env_key": "NAGA_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Navy API",
        "base_url": "https://api.navy/v1",
        "model": "deepseek-v4-flash",
        "env_key": "NAVY_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "NVIDIA NIM",
        "base_url": "https://integrate.api.nvidia.com/v1",
        "model": "deepseek-ai/deepseek-v4-flash",
        "env_key": "NVIDIA_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Ollama Cloud",
        "base_url": "https://api.ollama.com",
        "model": "gpt-oss:120b",
        "env_key": "OLLAMA_API_KEY",
        "endpoint": "/v1/chat/completions",
    },
    {
        "name": "OpenCode Zen",
        "base_url": "https://opencode.ai/zen/v1",
        "model": "deepseek-v4-flash-free",
        "env_key": "ZEN_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "OpenRouter",
        "base_url": "https://openrouter.ai/api/v1",
        "model": "nvidia/nemotron-nano-12b-v2-vl:free",
        "env_key": "OPENROUTER_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Poixe AI",
        "base_url": "https://api.poixe.com/v1",
        "model": "qwen3-235b-a22b-instruct-2507:free",
        "env_key": "POIXE_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Poolside",
        "base_url": "https://inference.poolside.ai/v1",
        "model": "poolside/laguna-m.1",
        "env_key": "POOLSIDE_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Routeway AI",
        "base_url": "https://api.routeway.ai/v1",
        "model": "laguna-xs.2:free",
        "env_key": "ROUTEWAY_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "SambaNova AI",
        "base_url": "https://api.sambanova.ai/v1",
        "model": "DeepSeek-V3.1",
        "env_key": "SAMBANOVA_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Speka AI",
        "base_url": "https://speka.me/v1",
        "model": "deepseek-ai/deepseek-v4-flash",
        "env_key": "SPEKA_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "TokenReply",
        "base_url": "https://api.tokenreply.com/v1",
        "model": "deepseek-v4-flash",
        "env_key": "TOKENREP_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Void AI",
        "base_url": "https://api.voidai.app/v1",
        "model": "deepseek-v4-pro",
        "env_key": "VOID_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Yolo-Auto",
        "base_url": "https://yolo-auto.com/v1",
        "model": "qwen3.6-35b-a3b",
        "env_key": "YOLO_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Z.AI (Zhipu AI)",
        "base_url": "https://api.z.ai/api/paas/v4",
        "model": "glm-4.7-flash",
        "env_key": "ZAI_API_KEY",
        "endpoint": "/chat/completions",
    },
    {
        "name": "Zylo API",
        "base_url": "https://api.zyloai.net/v1",
        "model": "deepseek-v4",
        "env_key": "ZYLO_API_KEY",
        "endpoint": "/chat/completions",
    },
]

TIMEOUT = 30
RUN_ALL = os.environ.get("VERIFY_ALL", "")


def test_openai(provider):
    """Test an OpenAI-compatible chat completion endpoint."""
    api_key = os.environ.get(provider["env_key"], "")
    if not api_key:
        return "SKIPPED", 0

    url = provider["base_url"].rstrip("/")
    endpoint = provider.get("endpoint", "/chat/completions")
    # Handle account_id placeholder
    if "{account_id}" in url:
        account_id_key = provider.get("account_id_key", "CLOUDFLARE_ACCOUNT_ID")
        account_id = os.environ.get(account_id_key, "")
        if not account_id:
            return "SKIPPED", 0
        url = url.replace("{account_id}", account_id)

    request_url = f"{url}{endpoint}"
    
    body = json.dumps({
        "model": provider["model"],
        "messages": [{"role": "user", "content": "Respond with OK"}],
        "max_tokens": 10,
        "stream": False,
    }).encode()

    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {api_key}",
    }

    req = urllib.request.Request(request_url, data=body, headers=headers, method="POST")
    
    start = time.time()
    try:
        with urllib.request.urlopen(req, timeout=TIMEOUT) as resp:
            elapsed = time.time() - start
            if resp.status == 200:
                return "OK", round(elapsed, 2)
            return f"HTTP {resp.status}", round(elapsed, 2)
    except urllib.error.HTTPError as e:
        elapsed = time.time() - start
        return f"HTTP {e.code}", round(elapsed, 2)
    except Exception as e:
        elapsed = time.time() - start
        return f"ERROR", round(elapsed, 2)


def test_gemini(provider):
    """Test Google Gemini API."""
    api_key = os.environ.get(provider["env_key"], "")
    if not api_key:
        return "SKIPPED", 0

    model_name = provider["model"]
    url = f"{provider['base_url'].rstrip('/')}/models/{model_name}:generateContent?key={api_key}"
    
    body = json.dumps({
        "contents": [{"parts": [{"text": "Respond with OK"}]}],
        "generationConfig": {"maxOutputTokens": 10},
    }).encode()

    headers = {"Content-Type": "application/json"}
    req = urllib.request.Request(url, data=body, headers=headers, method="POST")

    start = time.time()
    try:
        with urllib.request.urlopen(req, timeout=TIMEOUT) as resp:
            elapsed = time.time() - start
            if resp.status == 200:
                return "OK", round(elapsed, 2)
            return f"HTTP {resp.status}", round(elapsed, 2)
    except urllib.error.HTTPError as e:
        elapsed = time.time() - start
        return f"HTTP {e.code}", round(elapsed, 2)
    except Exception as e:
        elapsed = time.time() - start
        return f"ERROR", round(elapsed, 2)


def test_cohere(provider):
    """Test Cohere API."""
    api_key = os.environ.get(provider["env_key"], "")
    if not api_key:
        return "SKIPPED", 0

    url = f"{provider['base_url'].rstrip('/')}/chat"
    
    body = json.dumps({
        "model": provider["model"],
        "message": "Respond with OK",
        "max_tokens": 10,
    }).encode()

    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {api_key}",
    }

    req = urllib.request.Request(url, data=body, headers=headers, method="POST")

    start = time.time()
    try:
        with urllib.request.urlopen(req, timeout=TIMEOUT) as resp:
            elapsed = time.time() - start
            if resp.status == 200:
                return "OK", round(elapsed, 2)
            return f"HTTP {resp.status}", round(elapsed, 2)
    except urllib.error.HTTPError as e:
        elapsed = time.time() - start
        return f"HTTP {e.code}", round(elapsed, 2)
    except Exception as e:
        elapsed = time.time() - start
        return f"ERROR", round(elapsed, 2)


def main():
    results = []
    passed = 0
    skipped = 0
    failed = 0

    for p in PROVIDERS:
        api_type = p.get("api_type", "openai")
        
        if api_type == "gemini":
            status, latency = test_gemini(p)
        elif api_type == "cohere":
            status, latency = test_cohere(p)
        else:
            status, latency = test_openai(p)

        results.append((p["name"], status, latency))

        if status == "OK":
            passed += 1
        elif status == "SKIPPED":
            skipped += 1
        else:
            failed += 1

        icon = "✅" if status == "OK" else "⏭️" if status == "SKIPPED" else "❌"
        print(f"  {icon} [{p['name']}] {status} ({latency}s)"
              if status != "SKIPPED" else
              f"  {icon} [{p['name']}] SKIPPED (set ${p['env_key']})")

    print(f"\n🏁 {passed} passed, {failed} failed, {skipped} skipped ({len(PROVIDERS)} total)")


if __name__ == "__main__":
    main()
